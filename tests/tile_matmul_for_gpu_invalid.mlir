// RUN: not tutorial-opt %s -tile-matmul-for-gpu="block-m=0 block-n=32" 2>&1 | FileCheck %s --check-prefix=NONPOSITIVE
// RUN: not tutorial-opt %s -tile-matmul-for-gpu="block-m=64 block-n=32" 2>&1 | FileCheck %s --check-prefix=TOO-MANY
