// Benchmark: b4_resnet_expand  (784x64) x (64x256) -> (784x256)
#include <algorithm>
#include <chrono>
#include <cstdint>
#include <cstdio>
...
extern "C" {
void _mlir_ciface_b4_resnet_expand(
    MemRefDescriptor<float,2> *result,
    MemRefDescriptor<float,2> *A,
    MemRefDescriptor<float,2> *B);
}
static float A_data[784*64], B_data[64*256], C_data[784*256];
int main() {
  for (int i=0;i<784*64;i++) A_data[i]=0.001f*(float)(i%100+1);
  for (int i=0;i<64*256;i++) B_data[i]=0.001f*(float)((i+3)%100+1);
  MemRefDescriptor<float,2> A={A_data,A_data,0,{784,64},{64,1}};
  MemRefDescriptor<float,2> B={B_data,B_data,0,{64,256},{256,1}};
  for (int i=0;i<30;i++) {
    MemRefDescriptor<float,2> C={C_data,C_data,0,{784,256},{256,1}};
    _mlir_ciface_b4_resnet_expand(&C,&A,&B);
  }
