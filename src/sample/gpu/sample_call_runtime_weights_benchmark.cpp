...
int main() {
  float *input = nullptr;
  float *inputShift = nullptr;
  float *weightT = nullptr;
  float *bias = nullptr;
  cudaMallocManaged(&input, 12 * sizeof(float), cudaMemAttachGlobal);
  cudaMallocManaged(&inputShift, 12 * sizeof(float), cudaMemAttachGlobal);
  cudaMallocManaged(&weightT, 20 * sizeof(float), cudaMemAttachGlobal);
  cudaMallocManaged(&bias, 5 * sizeof(float), cudaMemAttachGlobal);

  float inputShiftHost[12] = {
  ...
  float biasHost[5] = {-0.0244604945f, -0.067863822f, 0.0572949648f,
                       0.481411994f, 0.292283773f};

  for (int i = 0; i < 12; ++i) {
    input[i] = 1.0f;
    inputShift[i] = inputShiftHost[i];
  }
  for (int i = 0; i < 20; ++i)
    weightT[i] = weightTHost[i];
  for (int i = 0; i < 5; ++i)
    bias[i] = biasHost[i];
  ...
  std::cout << "GPU avg inference time: " << std::fixed
            << std::setprecision(9) << (elapsed.count() / runs) << " sec\n";

  _mlir_ciface_sample_model(&outputRef, &inputRef, &shiftRef, &weightRef,
                            &biasRef);
  cudaDeviceSynchronize();
  if (outputRef.aligned) {
